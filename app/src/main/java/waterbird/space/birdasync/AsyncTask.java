...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    /*
        ...
        SynchronousQueue内部并没有数据缓存空间，你不能调用peek()方法来看队列中是否有数据元素，因为数据元素只有当你试
        着取走的时候才可能存在，不取走而只想偷窥一下是不行的，当然遍历这个队列的操作也是不允许的。队列头元素是第一个排队
        要插入数据的线程，而不是要交换的数据。数据是在配对的生产者和消费者线程之间直接传递的，并不会将数据缓冲数据到队列中。
	...
	/*
		handler的类别：
			1、不需要刷新UI，只处理消息，
			2、需要刷新UI，handler要用到主线程的Looper对象
		new Handler()，会默认用当前线程的Looper对象，
	 */
    ...
    /**
     * AsyncTask在android2.3的时候线程池是一个核心数为5线程，队列可容纳10线程，最大执行128个任务，
     *  这存在一个问题，当你真的有138个并发时，即使手机没被你撑爆，那么超出这个指标应用绝对crash掉。
     *  后来升级到4.0，为了避免并发带来的一些列问题，AsyncTask竟然成为序列执行器了，也就是你即使你
     *  同时execute N个AsyncTask，它也是挨个排队执行的。 这一点请同学们一定注意，<bold>AsyncTask在4.0
     *  以后，是异步的没错，但不是并发的。</bold>
     */
	...
	public AsyncTask() {
		...
		//将mWorker作为构造函数的参数去实例化FutureTask,FutureTask一个可取消的异步计算,FutureTask一个可取消的异步计算
